# ai/train.py (modified for new structure)
import torch
...
def battle_simulation(net, mcts_sims, render=False):
    ...
    game.receive_garbage(garbage_lines)
    game.receive_garbage(garbage_lines)
    print(f"[Simulation] Added {garbage_lines} garbage lines at start.")  # 可选日志
    if render:
        game.enable_render()
    ...
    while True:
        ...
        # 记录数据
        # 注意：这里需要重新获取 state，或者直接用上面的 board, ctx
        # 为了代码清晰，重新获取一次 current state 存入 buffer
        s_board, s_ctx = game.get_state()
        training_data.append([s_board, s_ctx, action_probs, None])
       
        # 选择动作
        legal = game.get_legal_moves()
        global_idx = np.random.choice(len(action_probs), p=action_probs)
        if root.legal_indices is None:
            raise ValueError("No legal indices")
